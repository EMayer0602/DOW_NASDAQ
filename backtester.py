    ...
    def run_backtest(self, symbol: str, df: pd.DataFrame) -> BacktestResult:
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            ...
            try:
                ...
                # If conversion fails, create a synthetic datetime index
                print(f"Warning: {symbol} has no datetime index, creating synthetic timestamps")
        ...
        for i in range(start_idx, len(df)):
            ...
            if pd.isna(timestamp) or not isinstance(timestamp, (pd.Timestamp, datetime)):
